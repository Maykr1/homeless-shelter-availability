...
class RhodeIslandDashboardAdapter:
    def __init__(self, settings: Settings) -> None:
        self.settings = settings

    async def collect_records(self) -> list[SourceShelterRecord]:
        if self.settings.import_fixture_mode:
            html = (self.settings.fixture_dir / "rhode_island_dashboard.html").read_text(encoding="utf-8")
            return self.parse_fixture_html(html)

        async with async_playwright() as playwright:
            browser = await playwright.chromium.launch(headless=True)
            page = await browser.new_page()
            await page.goto(self.settings.rhode_island_dashboard_url, wait_until="networkidle")
    ...
    def parse_fixture_html(self, html: str) -> list[SourceShelterRecord]:
        match = re.search(
            r'<script[^>]+id="ri-bed-data"[^>]*>(?P<data>.*?)</script>',
            html,
